...
public class 统计最高分中的节点数目 {
    ...
    int[][] buildTree(int[] parents){
        ...
        //根据parents数组构建二叉树(跳过parents[0]节点)
        for (int i = 1; i <n ; i++) {
    ...
    int countNode(int root){
        ...
        //二叉树中节点总数
        int n =tree.length;
        int leftTree = countNode(tree[root][0]);
        int rightTree = countNode(tree[root][1]);
        //后序位置，计算每个节点的分数
        int otherCount = n - leftTree - rightTree - 1;
        // 注意，这里要把 int 转化成 long，否则会产生溢出！！！
        long score =(long) Math.max(leftTree,1) * Math.max(rightTree,1) * Math.max(otherCount,1);
        //给分数 score 计数
