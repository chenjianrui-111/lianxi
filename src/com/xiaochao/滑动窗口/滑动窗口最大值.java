 ...
 * 由于只涉及单点修改，无须实现懒标记 pushdown 操作，再结合 nnn 的数据范围为 105，无须进行动态开点。
...
class Solution3{
    ...
    public int[] maxSlidingWindow2(int[] nums, int k) {
        ...
        for (int i = k; i <= n ; i++) {
            ans[i - k] = query(1,i - k + 1,1);
        }
        return ans;
    }
}
//时间复杂度：建立线段树复杂度为O(nlogn)；构建答案复杂度为 O(nlogn)。整体复杂度为 O(nlogn)
//空间复杂度：O(n)
/**
 * 分块
 ...
 * 我们可以设定块大小为 sqrt{k}
 * 这样所需创建的分块数组大小为 n/sqrt{k}
 * 分块数组的更新操作为 O(1)，而查询则为 k/sqrt{k}
 * 容易证明查询操作的复杂度：对于每个长度为 k 的 [l,r] 查询操作而言，最多遍历两个（左右端点对应的块）的块内元素，复杂度为 O(k)
 *  同时最多遍历 k/sqrt{k}个块，复杂度同为 O(k)
 * 因此最多两步复杂度为 O(k)O(\sqrt{k})的块内操作，最多 k/sqrt{k}
 * 步复杂度为 O(1) 的块间操作，整体复杂度为 O(sqrt{k})
 * 因此使用分块算法总的计算量为 n×k=106n\times\sqrt{k} = 10^6 可以过。
 * 分块算法的几个操作函数：
...
class Solution4{
    ...
    int query(int l ,int r){
        ...
        if (getIdx(l) == getIdx(r)){
            ...
            while (getIdx(j) == getIdx(r)) ans = Math.max(ans , nums[i--]);
            for (int k = getIdx(i); k <= getIdx(j) ; k++) {
                ans = Math.max(ans,region[k]);
            }
        }
        return ans;
    }
...
/**
 *时间复杂度：数组大小为 n，块大小为 sqrt{k}
 * 分块数组大小为 n/sqrt{k}
 * 预处理分块数组复杂度为O(n)（即 add 操作复杂度为 O(1)）；构造答案复杂度为 O(n*sqrt{k})
 * （即 query 操作复杂度为 O(\sqrt{k})，最多有 n 次查询）
 * 空间复杂度：n/sqrt{k}
 */
class Solution5{
    /**
     ...
     * 随着窗口的不断平移，该过程会一直发生。若同一时刻存在两个数nums[j] 和 nums[i]（j < i）所在一个窗口内，下标更大的数会被更晚移出窗口，
     * 此时如果有 nums[j]<=nums[i] 的话，可以完全确定 nums[j] 将不会成为后续任何一个窗口的最大值，此时可以将必然不会是答案的nums[j] 从候选中进行移除。
     * 不难发现，当我们将所有必然不可能作为答案的元素（即所有满足的小于等于 nums[i]）移除后，候选集合满足「单调递减」特性，即集合首位元素为当前窗口中的最大值
     * （为了满足窗口长度为 k 的要求，在从集合头部取答案时需要先将下标小于的等于的 i−k 的元素移除）。
