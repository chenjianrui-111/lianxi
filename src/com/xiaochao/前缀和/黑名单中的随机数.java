 ...
 * 否则需要在 [n-m,n-1] 内找到一个可替代它的数值，我们可以使用一个变量 idx 在[n-m,n-1]  范围内从前往后扫描，找到第一个未被使用的，
 * 同时不在 s2 中（不在黑名单内）的数字，假设找到的值为 x，将 x 进行返回（同时将 val 与 x 的映射关系，用哈希表进行记录）。
 */
class Solution {
    int n,m,idx;
    ...
    public Solution(int _n, int[] _blacklist) {
        n = _n; m = _blacklist.length;
        int max = n - m;
        for (int x : _blacklist) {
            if (x < max) s1.add(x);
            else s2.add(x);
        }
        idx = n - m;
 ...
 * 具体的，我们先对 bs 进行排序，然后从前往后处理所有的 bbs[i]，将相邻 bs[i] 之间的能被选择的「线段」以二元组 (a, b) 的形式进行记录（即一般情况下的 a = bs[i - 1] + 1，b = bs[i] - 1），存入数组 list 中（注意特殊处理一下两端的线段）。
...
///时间复杂度：在初始化操作中：对 bs 进行排序复杂度为 O(mlogm)；统计所有线段复杂度为 O(m)；对所有线段求前缀和复杂度为 O(m)。
// 在 pick 操作中：随机后会对所有线段做二分，复杂度为 O(logm)
//空间复杂度：O(m)
