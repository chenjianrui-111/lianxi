...
/**
 *记忆化搜索
 * 不难发现，在 DFS 的函数签名中只有「数值下标 u 」和「当前结算结果 cur 」为可变参数，
...
//• 时间复杂度：O(n ∗ ∑i=0 n−1abs(nums[i]))
//• 空间复杂度：忽略递归带来的额外空间消耗。复杂度为 O(n ∗ ∑i=0 n−1abs(nums[i]))
/**
 *动态规划
 * 能够以「递归」的形式实现动态规划（记忆化搜索），自然也能使用「递推」的方式进行实现。
 * 根据记忆化搜索的分析，我们可以定义：
 * f[i][j] 代表考虑前 i 个数，当前计算结果为 j 的方案数，令 nums 下标从 1 开始。
 * 那么 f[n][target] 为最终答案，f[0][0] = 1 为初始条件：代表不考虑任何数，凑出计算结果 为 0 的方案数为 1 种。
 ...
 * • 第二维为中间结果：令 s 为所有 nums 元素的总和（题目给定了 nums[i] 为非 负数的条件，否则需要对 nums[i] 取绝对值再累加），那么中间结果的范围为[−s, s]
 * 因此，我们可以确定动规数组的大小。同时在转移时，对第二维度的使用做一个 s 的右偏移，
 * 以确保「负权值」也能够被合理计算/存储。
 */
class Solution13{
    public int findTargetSumWays(int[] nums, int t) {
...
//• 时间复杂度：O(n ∗ ∑i=0 n−1 abs(nums[i]))
//• 空间复杂度：O(n ∗ ∑i=0 n−1 abs(nums[i]))
/**
 *动态规划（优化）
 * 在上述「动态规划」分析中，我们总是尝试将所有的状态值都计算出来，当中包含很多对「目标 状态」不可达的“额外”状态值。
 * 即达成某些状态后，不可能再回到我们的「目标状态」。
 * 例如当我们的 target 不为 −s 和 s 时，−s 和 s 就是两个对「目标状态」不可达的“额外”状态
 * 值，到达 −s 或 s 已经使用所有数值，对 target 不可达。
 * 那么我们如何规避掉这些“额外”状态值呢？
 * 我们可以从哪些数值使用哪种符号来分析，即划分为「负值部分」&「非负值部分」，令「负值
 * 部分」的绝对值总和为 m，即可得：(s − m) − m = s − 2 ∗ m = target
 * 变形得： m = (s − target) / 2
 * 问题转换为：只使用 + 运算符，从 nums 凑出 m 的方案数。
 * 这样「原问题的具体方案」和「转换问题的具体方案」具有一一对应关系：「转换问题」中凑出
 * 来的数值部分在实际计算中应用 −，剩余部分应用 +，从而实现凑出来原问题的 target 值。
 * 另外，由于 nums 均为非负整数，因此我们需要确保 s − target 能够被 2 整除。
 * 同时，由于问题转换为 从 nums 中凑出 m 的方案数，因此「状态定义」和「状态转移」都需
 * 要进行调整（01 背包求方案数）：
 * 定义 f[i][j] 为从 nums 凑出总和「恰好」为 j 的方案数。
 * 最终答案为 f[n][m]，f[0][0] = 1 为起始条件：代表不考虑任何数，凑出计算结果为 0 的方
 * 案数为 1 种。
 * 每个数值有「选」和「不选」两种决策，转移方程为： 代码：  f[i][j] = f[i − 1][j] + f[i − 1][j − nums[i − 1]]
 */
class Solution14 {
    public int findTargetSumWays(int[] nums, int t) {
        int n = nums.length;
...
//• 时间复杂度：O(n ∗ (∑i=0 n−1 2abs(nums[i]) − target))
//• 空间复杂度：O(n ∗ (∑i=0 n−1 abs(nums[i]) − target))
