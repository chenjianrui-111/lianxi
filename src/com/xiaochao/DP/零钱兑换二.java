 ...
 * 对于第 i 个硬币我们有两种决策方案：
 * • 不使用该硬币：f[i − 1][j]
 * • 使用该硬币：由于每个硬币可以被选择多次（容量允许的情况下），因此方案数量
 * 应当是选择「任意个」该硬币的方案总和：
 ...
 *完全背包（一维优化）
 * 显然二维完全背包求解方案复杂度有点高。
 * n 的数据范围为 102，cnt 的数据范围为 103，总的计算量为 108 以上，处于超时边缘（实际测试可通过）。
 * 我们需要对其进行「降维优化」，可以使用最开始讲的 数学分析方式，或者上一讲讲的 换元优
 * 化方式 进行降维优化。
...
//• 时间复杂度：共有 n ∗ cnt 个状态需要转移，整体复杂度为 O(n ∗ cnt)。 • 空间复杂度：O(cnt)。
