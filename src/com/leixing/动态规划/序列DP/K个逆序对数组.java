 ...
 * 不失一般性的考虑 f[i][j]该如何计算，对第 ii 个数（即数值为 i 的数）所在位置进行讨论，共有 i 种选择。
 * 假设第 i 个数所在位置为 k，由于数值 i 为整个数组的最大值，因此数值 i 与前面所有数均不形成逆序对，与后面的所有数均形成逆序对。因此与数值 ii 直接相关的逆向对的数量为 (i - 1)- k(i−1)−k，由此也得出与 ii 不相关的逆序对数量为 j - (i - 1 - k)j−(i−1−k)，而与 ii 不相关的逆序对数量由 f[i - 1][x]f[i−1][x] 可得出。
 * 举个 🌰 帮助大家理解：
 * 当数值 i 放置在下标为 0 的位置上，那么由数值 i 产生的逆序对数量为 i - 1，总的逆序对数量为 j，因此由数值范围为 [1, i - 1]（与数值 ii 不相关）构成的逆序对数量为 j - (i - 1)j−(i−1)，即 f[i - 1][j - (i - 1)]；
 ...
 * 共有 n * kn∗k 个状态，每个 f[i][j]f[i][j] 的计算需要枚举数值 ii 所在位置并进行累加，总的复杂度为 O(n^2 *k)计算量为 10^9
 * 会 TLE。
 * 状态数量不可减少，考虑如何优化单个状态的转移过程。
 * 不难发现 \sum_{k = 0}^{i - 1}(f[i - 1][j - (i - 1 - k)])
 *  (f[i−1][j−(i−1−k)]) 部分为上一次转移结果 f[i - 1][x]的某个前缀，可以使用前缀和数组进行优化，从而将计算单个状态的复杂度从 O(n) 降到 O(1)。
