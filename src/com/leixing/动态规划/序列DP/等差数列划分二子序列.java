 ...
 * 先从最朴素的猜想出发，定义 f[i] 为考虑下标不超过 ii 的所有数，并且以 nums[i] 为结尾的等差序列的个数。
 ...
 * 我们可以设置 f[i] = g，其中 g 为一个「集合」数据结构，我们期望在O(1) 的复杂度内查的某个差值 d 的子序列个数是多少。
 ...
 * 含义为：在原本以 nums[j] 为结尾的，且差值为 dd 的子序列的基础上接上 nums[i]，再加上新的子序列 (nums[j], nums[i])，共 f[j][d] + 1 个子序列。
 * 最后对所有的哈希表的「值」对进行累加计数，就是以任意位置为结尾，长度大于 1 的等差子序列的数量 ans。
 * 这时候再看一眼数据范围 -2^{31} <= nums[i] <= 2^{31}-1
 *  如果从数据范围出发，使用「数组」充当集合的话，我们需要将数组开得很大，必然会爆内存。
 * 但同时有 1 <= nums.length <= 1000，也就是说「最小差值」和「最大差值」之间可能相差很大，但是差值的数量是有限的，不会超过 n^2
 * 为了不引入复杂的「离散化」操作，我们可以直接使用「哈希表」来充当「集合」。
 * 每一个 f[i] 为一个哈希表，哈希表的以 {d:cnt} 的形式进行存储，d 为子序列差值，cnt 为子序列数量。
 * 虽然相比使用数组，哈希表常数更大，但是经过上述分析，我们的复杂度为 O(n^2)
 * 计算量为 10^6
 * 距离计算量上界 10^7
 * 还保有一段距离，因此直接使用哈希表十分安全。
 ...
 * 而求长度为 n 的数组的所有数对，其实就是求 首项为 0，末项为 n - 1公差为 1，长度为 n 的等差数列之和，直接使用「等差数列求和」公式求解即可。
 */
public class 等差数列划分二子序列 {
