 ...
 * 因此我们的状态转移方程为：
 * f[i][j]=min(f[i−1][j−k∗t]+k),0⩽k∗t⩽j
 * 当然，能够选择 k 个数字 i 的前提是，剩余的数字 j - k * t也能够被其他「完全平方数」凑出，即 f[i - 1][j - k * t]为有意义的值。
...
//时间复杂度：预处理出所有可能用到的数字复杂度为 O(\sqrt{n})共有 n * \sqrt{n} 个状态需要转移，每个状态转移最多遍历 nn 次，因此转移完所有状态复杂度为 O(n^2 * \sqrt{n})整体复杂度为 O(n^2 * \sqrt{n})O
//空间复杂度：O(n * \sqrt{n})
